...
def get_weighted_map(train,train_mask):
    ...
    out = train_map_save*train
    return out

train = np.load('E:\D3_map_segmentation\data\ed_img.npy')
train_mask = np.load('E:\D3_map_segmentation\data\ed_gt.npy')

out = get_weighted_map(train, train_mask)
np.save('E:\D3_map_segmentation\data\\ed_weighted_map.npy', out)

for i in range(out.shape[0]):#out.shape[0]
  img = out[i,:,:,:]
  img = array_to_img(img)
  img.save('E:\D3_map_segmentation\data\ED\weighted_map\patient%d.jpg'%i)
