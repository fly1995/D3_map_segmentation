...
from keras.layers import Dense, Dropout,Input, Activation, concatenate, Conv2D, MaxPooling2D, Conv2DTranspose, ZeroPadding2D,UpSampling2D, GlobalAveragePooling2D
...
def dice_coef_loss(y_true, y_pred):
    ...
    return -(dice_coef(y_true, y_pred)+myo(y_true, y_pred)+lv(y_true, y_pred)+rv(y_true, y_pred))
...
def fcn(input_shape=(shape1), num_classes=4):
    ...
    pool1 = MaxPooling2D(pool_size=3, strides=2,padding='same', name='pool1')(mvn3)
    ...
    pool2 = MaxPooling2D(pool_size=3, strides=2,padding='same', name='pool2')(mvn7)
    ...
    pool3 = MaxPooling2D(pool_size=3, strides=2,padding='same', name='pool3')(mvn11)
    ...
    score_conv15 = Conv2D(filters=num_classes, kernel_size=1,strides=1, activation=None, padding='same',kernel_initializer='glorot_uniform', use_bias=True,name='score_conv15')(drop2)
    upsample1 = Conv2DTranspose(filters=num_classes, kernel_size=3, strides=2, activation=None, padding='same', kernel_initializer='glorot_uniform', use_bias=False,name='upsample1')(score_conv15)
    score_conv11 = Conv2D(filters=num_classes, kernel_size=1,strides=1, activation=None, padding='same',kernel_initializer='glorot_uniform', use_bias=True,name='score_conv11')(mvn11)
    crop1 = Lambda(crop, name='crop1')([upsample1, score_conv11])
    fuse_scores1 = average([crop1, upsample1], name='fuse_scores1')

    upsample2 = Conv2DTranspose(filters=num_classes, kernel_size=3,strides=2, activation=None, padding='same',kernel_initializer='glorot_uniform', use_bias=False,name='upsample2')(fuse_scores1)
    score_conv7 = Conv2D(filters=num_classes, kernel_size=1,strides=1, activation=None, padding='same',kernel_initializer='glorot_uniform', use_bias=True,name='score_conv7')(mvn7)
    crop2 = Lambda(crop, name='crop2')([upsample2, score_conv7])
    fuse_scores2 = average([crop2, upsample2], name='fuse_scores2')

    upsample3 = Conv2DTranspose(filters=num_classes, kernel_size=3,strides=2, activation=None, padding='same',kernel_initializer='glorot_uniform', use_bias=False,name='upsample3')(fuse_scores2)
    crop3 = Lambda(crop, name='crop3')([data, upsample3])
    predictions = Conv2D(filters=num_classes, kernel_size=1,strides=1, activation='softmax', padding='same',kernel_initializer='glorot_uniform', use_bias=True,name='predictions')(crop3)
...
def MDFA_Net(x = Input(shape=shape1),num_classes=1,features=32, depth=4, padding='same', kernel_size = (3, 3)):
    ...
    pad = ZeroPadding2D(padding=0, name='pad')(mvn0)
    ...
    score_conv15 = Conv2D(filters=num_classes, kernel_size=1, strides=1, activation=None, padding='same',
                          kernel_initializer='glorot_uniform', use_bias=True, name='score_conv15')(drop2)
    upsample1 = Conv2DTranspose(filters=num_classes, kernel_size=3, strides=2, activation=None, padding='same',
                                kernel_initializer='glorot_uniform', use_bias=False, name='upsample1')(score_conv15)
    score_conv11 = Conv2D(filters=num_classes, kernel_size=1, strides=1, activation=None, padding='same',
                          kernel_initializer='glorot_uniform', use_bias=True, name='score_conv11')(mvn11)
    crop1 = Lambda(crop, name='crop1')([upsample1, score_conv11])
    fuse_scores1 = average([crop1, upsample1], name='fuse_scores1')

    upsample2 = Conv2DTranspose(filters=num_classes, kernel_size=3, strides=2, activation=None, padding='same',
                                kernel_initializer='glorot_uniform', use_bias=False, name='upsample2')(fuse_scores1)
    score_conv7 = Conv2D(filters=num_classes, kernel_size=1, strides=1, activation=None, padding='same',
                         kernel_initializer='glorot_uniform', use_bias=True, name='score_conv7')(mvn7)
    crop2 = Lambda(crop, name='crop2')([upsample2, score_conv7])
    fuse_scores2 = average([crop2, upsample2], name='fuse_scores2')

    upsample3 = Conv2DTranspose(filters=num_classes, kernel_size=3, strides=2, activation=None, padding='same',
...
def DCNN(input_size=(160, 160, 1)):
    inputs = Input(input_size)
    conv1 = Conv2D(16, (9, 9), dilation_rate=1, activation='relu', padding='same', kernel_initializer='he_normal')(inputs)
    conv1 = Activation('relu')(BatchNormalization()(conv1))

    conv2 = Conv2D(64, (7, 7), dilation_rate=2, activation='relu', padding='same', kernel_initializer='he_normal')(conv1)
    conv2 = Activation('relu')(BatchNormalization()(conv2))

    conv3 = Conv2D(64, (5, 5), dilation_rate=4, activation='relu', padding='same', kernel_initializer='he_normal')(conv2)
    conv3 = Activation('relu')(BatchNormalization()(conv3))

    conv4 = Conv2D(64, (3, 3), dilation_rate=6, activation='relu', padding='same', kernel_initializer='he_normal')(conv3)
    conv4 = Activation('relu')(BatchNormalization()(conv4))

    conv5 = Conv2D(64, (3, 3), dilation_rate=8, activation='relu', padding='same', kernel_initializer='he_normal')(conv4)
    conv5 = Activation('relu')(BatchNormalization()(conv5))

    output = Conv2D(4, (1, 1), activation='softmax')(conv5)
    model = Model(inputs=[inputs], outputs=[output])
    model.compile(optimizer=Adam(lr=0.0001), loss=[dice_coef_loss], metrics=[dice_coef, myo, lv, rv])
    model.summary()
    return model
...
def path1(x = Input(shape=shape1),features=16, depth=4, padding='same', dilation_rate = 1, kernel_size = (3, 3)):
    inputs = x
    maps = [inputs]
    ib = IB3(inputs,features*2)
